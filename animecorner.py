...
def get_news_links(page_url):
    ...
    list_news = soup.find_all('article', {'class': 'hentry'})
    for news in list_news:
        news_url = news.find('div',{'class':'header-list-style'}).find('h2',{'class':'entry-title'}).find('a')['href']
...
def get_news(news_url):
    ...
    # Remove noscript div
    noscript_divs = content_div.find_all('noscript')
    for div in noscript_divs:
        div.unwrap()
